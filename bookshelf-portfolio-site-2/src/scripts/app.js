(function () {
  ...
  function decorativeRun(count, offset) {
    ...
    for (var index = 0; index < count; index += 1) {
      ...
      if (sequence % 19 === 6) {
        lean = "left";
      } else if (sequence % 23 === 11) {
        lean = "right";
      }

      books.push(decorativeBook(
        DECORATIVE_COLORS[sequence % DECORATIVE_COLORS.length],
        DECORATIVE_HEIGHTS[(sequence * 3 + 1) % DECORATIVE_HEIGHTS.length],
        DECORATIVE_WIDTHS[(sequence * 5 + 2) % DECORATIVE_WIDTHS.length],
        DECORATIVE_DETAILS[(sequence * 2 + 1) % DECORATIVE_DETAILS.length],
        lean
      ));
    }
  ...
  /* Three fixed, densely filled rows keep the cabinet feeling hand-arranged. */
  var SHELF_LAYOUT = Object.freeze([
    shelfRow(
      decorativeRun(8, 0),
      sectionBook("about", "aubergine", "tall", "regular", "gilt"),
      decorativeRun(9, 8),
      prop("plant", "hanging"),
      decorativeRun(10, 17),
      sectionBook("experience", "maroon", "medium", "regular", "gilt"),
      prop("candle"),
      sectionBook("skills", "gold", "tall", "slim", "gilt"),
      decorativeRun(9, 27)
    ),
    shelfRow(
      decorativeRun(7, 36),
      ...
      prop("globe"),
      decorativeRun(7, 50),
      sectionBook("research", "aubergine", "medium", "regular", "gilt"),
      decorativeRun(7, 57),
      prop("teacup"),
      decorativeRun(6, 64),
      sectionBook("education", "maroon", "tall", "regular", "gilt"),
      decorativeRun(3, 70)
    ),
    shelfRow(
      decorativeRun(8, 73),
      sectionBook("resume", "gold", "tall", "regular", "gilt"),
      decorativeRun(8, 81),
      prop("quill"),
      decorativeRun(8, 89),
    ...
    SHELF_LAYOUT.forEach(function (items, rowIndex) {
      ...
      items.forEach(function (item, itemIndex) {
        ...
        contents.appendChild(
          item.type === "prop"
            ? createProp(item)
            : createBook(item, content, rowIndex, itemIndex, currentDecorativeOrdinal)
        );
      });
  ...
  function createBook(item, content, rowIndex, itemIndex, decorativeOrdinal) {
    ...
    var classes = [
      ...
      "book--pixel-" + (((rowIndex * 7 + itemIndex) % 6) + 1)
    ];
