...
assert.ok(fillerCount >= 90, `Shelves should feel packed; found only ${fillerCount} filler books`);

for (const iconName of ["plant", "candle", "teacup", "globe", "quill", "favicon"]) {
...
assert.ok(shelfLayout.every((row) => row.length >= 40), "Each shelf must contain at least 40 books and props");
assert.equal(
...
assert.equal(
  ...
  JSON.stringify(["candle", "globe", "plant", "quill", "teacup"]),
  "The intended five shelf props must each appear once"
);
