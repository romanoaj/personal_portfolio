...
The wood texture was generated specifically for this project from the supplied visual references. The prop icons and favicon are original SVG pixel art constructed from non-overlapping, integer-aligned grid cells. The books are CSS-rendered pixel art rather than image-generation assets, so their proportions, labels, and colors remain easy to tune. No external image or font requests are required.
