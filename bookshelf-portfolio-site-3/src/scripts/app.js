(function () {
  ...
  function bookend(side) {
    return { type: "bookend", side: side };
  }
  ...
  var SHELF_LAYOUT = Object.freeze([
    ...
    shelfRow(
      ...
      bookend("right"),
      decorativeRun(2, 52),
      sectionBook("education", "maroon", "tower", "wide", "gilt"),
      decorativeRun(4, 54),
    ...
    SHELF_LAYOUT.forEach(function (items, rowIndex) {
      ...
      items.forEach(function (item, itemIndex) {
        ...
        if (item.type === "prop") {
          ...
          contents.appendChild(createBookend(item));
  ...
  function createBookend(item) {
    var wrapper = document.createElement("span");
    var image = document.createElement("img");

    wrapper.className = "shelf-bookend shelf-bookend--" + item.side;
    wrapper.setAttribute("aria-hidden", "true");
    image.src = "assets/icons/bookend.svg";
    image.alt = "";
    image.width = 48;
    image.height = 48;
    image.loading = "eager";
  ...
  function createHorizontalStack(item, rowIndex, itemIndex) {
    ...
    stack.className = "horizontal-stack";
