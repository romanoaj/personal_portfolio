...
sprite("sleeping-cat", ({ set, row, column, colors: c }) => {
  ...
  // The white chest and two separate forepaws support the cat's chin.
  row(16,18,23,c.outline); row(17,18,24,c.outline); row(18,17,24,c.outline);
  row(19,17,24,c.outline); row(20,17,23,c.outline); row(21,18,22,c.outline);
  row(17,19,22,c.catWhite); row(18,18,23,c.catWhite);
  row(19,18,23,c.catWhiteShadow); row(20,18,22,c.catWhiteShadow);

  // Each foreleg has its own outline and toe marks instead of merging into one block.
  row(19,4,10,c.outline); row(20,3,11,c.outline);
  row(21,2,11,c.outline); row(22,2,11,c.outline); row(23,2,11,c.outline);
  row(24,2,11,c.outline); row(25,2,11,c.outline); row(26,3,10,c.outline);
  row(27,4,9,c.outline);
  row(20,5,9,c.catWhite); row(21,4,9,c.catWhite);
  row(22,3,9,c.catWhite); row(23,3,9,c.catWhite);
  row(24,3,9,c.catWhite); row(25,3,9,c.catWhiteShadow);
  row(26,4,9,c.catWhiteShadow);

  row(19,11,17,c.outline); row(20,10,19,c.outline);
  row(21,10,20,c.outline); row(22,10,20,c.outline); row(23,10,20,c.outline);
  row(24,10,20,c.outline); row(25,10,20,c.outline); row(26,11,19,c.outline);
  row(27,12,18,c.outline);
  row(20,12,17,c.catWhite); row(21,12,18,c.catWhite);
  row(22,12,19,c.catWhite); row(23,12,19,c.catWhite);
  row(24,12,19,c.catWhite); row(25,12,19,c.catWhiteShadow);
  row(26,12,18,c.catWhiteShadow);
  [[7,25],[8,25],[15,25],[16,25]].forEach(([x,y]) => set(x,y,c.outline));
