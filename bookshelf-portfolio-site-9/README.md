...
An original, dependency-free one-page portfolio presented as a whimsical pixel-art bookshelf. Exactly three tall, cluttered shelves hold varied grid-drawn book spines, two horizontal book stacks, loose pages, sturdy wood supports, a wide five-rung library ladder with a sleeping tuxedo cat, and small pixel props. Titled books open accessible book-shaped dialogs for About Me, Education, Skills, Experience, Research, Projects, Hobbies, Resume, and Contact.
...
- Edit the display name, nameplate instruction, and social-preview metadata in `index.html`.
...
To rebuild the strict integer-grid sprites after changing `tools/build_pixel_sprites.mjs`, run:
...
- The native dialog supports Escape, backdrop click, focus return, keyboard focus containment, and keyboard-scrollable book pages.
- An accessible hamburger catalog provides an alternative to finding titles on the shelves.
- Reduced-motion and high-contrast preferences are respected.

## Artwork

The wood texture was generated specifically for this project from the supplied visual references. The upright sprout, up-and-over pothos, candle, half-arm globe, steaming teacup, quill, amethyst geode, strawberry picture frame, sleeping tuxedo cat, swinging tail, and favicon are original SVG pixel art constructed from non-overlapping, integer-aligned grid cells. The ladder and books are CSS-rendered pixel art rather than image-generation assets, so their proportions, labels, band placement, and colors remain easy to tune. No external image or font requests are required.
